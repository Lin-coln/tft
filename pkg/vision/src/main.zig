...
const allocNSDataFromBytes = @import("img.zig").allocNSDataFromBytes;
...
fn run(init: std.process.Init) !void {
    ...
    const image_data = allocNSDataFromBytes(png_bytes) catch |err| {
        report(init.io, "failed to create image data for '{s}': {s}\n", .{ path, @errorName(err) });
        return error.Reported;
    };
    defer image_data.release();

    const inference = Infer.init(image_data);
    defer inference.deinit();

    inference.perform() catch |err| {
        report(init.io, "failed to recognize text in '{s}': {s}\n", .{ path, @errorName(err) });
        return error.Reported;
    };
    const result = inference.readResults(allocator) catch |err| {
        report(init.io, "failed to read text results for '{s}': {s}\n", .{ path, @errorName(err) });
        return error.Reported;
    };
