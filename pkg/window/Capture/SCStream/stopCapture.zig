...
pub fn stopCapture(stream: objc.Object, ctx: ?*anyopaque, callback: Callback) void {
    var block = Block.init(.{ .ctx = ctx, .callback = @intFromPtr(callback) }, &completed);

    stream.msgSend(void, "stopCaptureWithCompletionHandler:", .{&block});
}
...
pub fn sync(stream: objc.Object) !void {
    ...
    stopCapture(stream, &state, &Sync.completed);

    _ = dispatch.semaphore_wait(semaphore, .FOREVER);

    if (state.failed) return error.StopCaptureFailed;
}
