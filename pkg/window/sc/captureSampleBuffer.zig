...
pub fn captureSampleBuffer(
    ...
    const manager = objc.getClass("SCScreenshotManager") orelse {
...
pub fn retain(filter: Object, config: Object) !Object {
    const Sync = struct {
        const State = struct {
    ...
    captureSampleBuffer(filter, config, &state, &Sync.completed);

    _ = dispatch.semaphore_wait(semaphore, .FOREVER);

    if (state.failed) return error.CaptureSampleBufferFailed;
    return state.data.?;
}
