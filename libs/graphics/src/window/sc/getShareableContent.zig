...
const Block = objc.Block(struct { ctx: ?*anyopaque, callback: usize }, .{ objc.c.id, objc.c.id }, void);
...
fn completed(block: *const Block.Context, data_id: objc.c.id, err_id: objc.c.id) callconv(.c) void {
    const callback: Callback = @ptrFromInt(block.callback);

    callback(
        block.ctx,
        if (data_id != null)
            Object.fromId(data_id)
        else
            null,
        if (err_id != null)
